 ...
 */
class DT_Webform_Migration_Engine
{
    /**
     ...
     *
     * @throws \Exception ...
     * @throws \DT_Webform_Migration_Lock_Exception ...
     * @throws \Throwable ...
     */
    public static function migrate( int $target_migration_number ) {
        ...
        while ( true ) {
            $current_migration_number = get_option( 'dt_webform_migration_number' );
            ...
            if ( (int) get_option( 'dt_webform_migration_lock', 0 ) ) {
                throw new DT_Webform_Migration_Lock_Exception();
            }
            update_option( 'dt_webform_migration_lock', '1' );

            error_log( date( " Y-m-d H:i:s T" ) . " Starting migrating to number $activating_migration_number" );
            try {
                $migration->up();
            } catch (Throwable $e) {
                update_option( 'dt_webform_migrate_last_error', [
            ...
            update_option( 'dt_webform_migration_number', (string) $activating_migration_number );
            error_log( date( " Y-m-d H:i:s T" ) . " Done migrating to number $activating_migration_number" );

            update_option( 'dt_webform_migration_lock', '0' );
...
class DT_Webform_Migration_Lock_Exception extends Exception
{
    public function __construct( $message = null, $code = 0, Exception $previous = null ) {
         ...
         */
        $last_migration_error = get_option( 'dt_webform_migrate_last_error' );
        if ($message === null) {
